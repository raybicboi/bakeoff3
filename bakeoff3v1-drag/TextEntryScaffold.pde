...
// variable for passing frames to change letters
int framesPassed = 0;
...
void mouseDragged() {
  framesPassed += 1;
  if (didMouseClick(width/2-sizeOfInputArea/2, height/2-sizeOfInputArea/2+sizeOfInputArea/2, sizeOfInputArea, sizeOfInputArea/2)) //check if click in left button
  {
    if (mouseX > pmouseX) {
      if (framesPassed % 5 == 0) {
        currentLetter ++;
